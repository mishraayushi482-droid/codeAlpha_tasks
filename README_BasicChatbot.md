...
- **Continuous Conversation Loop:**
  Keeps the chat session alive using a robust 'while' loop until closed by the user.
- **Smart Rule-Based Logic:**
  Evaluates multiple input Conditions by using optimized 'if-elif-else branching.' 
- **User-Friendly Interface:**
  Clear Terminal prompts that distinguish between User Inputs(You:) and ChatBot Response(Chatbot:)
- **Input Sanitization:**
  Automatically Handles Case-Insensitivity (e.g. "HELLO", "Hello", and "hello" all work perfectly.)
